	...
	<?php 
		...
		function highlight($word, $context){
			...
			$highlightedWith = array('<span style="background-color: gold; color: black;">'.$context.'</span>', ' ');
	...
	?>
			...
			<?php foreach($media->result() as $row): ?>
				<?php if($row->image != ""):?>
					...
					<?php $mediaid = $row->id-1;?>
						...
						<p><strong><a href="<?=base_url();?><?php echo "media_center/media#prettyPhoto[gallery1]/".$mediaid."/";?>"><?php echo highlight($row->title, $keyword);?></a></strong></p>
						<p><?php echo highlight($row->description, $keyword);?></p>
					</div>
					<br/>
				<?php $section_empty1 = ""; ?> 
				<?php endif; ?> 
			...
			<?php foreach($media->result() as $row): ?>
				<?php if($row->audio != ""):?>
					...
					<?php $mediaid = $row->id-1;?>
						...
						<p><strong><a href="<?=base_url();?><?php echo "media_center#audio";?>"><?php echo highlight($row->title, $keyword);?></a></strong></p>
						<p><?php echo highlight($row->description, $keyword);?></p>
					</div>
					<br/>
				<?php else:	
					...
					<?php $newsid = $row->id-1;?>
						<div class="rounded"  style="margin: 10px; width: 200px !important; height: 200px !important; background: black !important;">
									...
									<span style="text-indent: 15px">
										<p><strong><a href="<?=base_url();?><?php echo "media_center#video";?>"><?php echo highlight($row->title, $keyword);?></a></strong></p>
										<p><?php echo highlight($row->description, $keyword);?></p>
									</span>
						</div>
					</div>
		...
		<?php if(($shop->num_rows() > 0)&&($keyword!="")):?>
			<?php foreach($shop->result() as $row): ?>
				<div class="container thin">
					...
					<p><strong><a href="<?=base_url();?><?php echo base_url(); ?>shop/product/<?php echo $row->category_id; ?>/<?php echo $row->id; ?>"><?php echo highlight($row->product_name, $keyword);?></a></strong></p>
					<p><?php echo highlight($row->product_description, $keyword);?></p>
				</div>
				<br/>
			<?php endforeach; ?>
		<?php else:	
		...
		<?php if(($news->num_rows() > 0)&&($keyword!="")):?>
			<?php foreach($news->result() as $row): ?>
				<div class="container thin">
				<?php $newsid = $row->id-1;?>
					<p><strong><a href="<?=base_url();?><?php echo base_url(); ?>news_blogs/blog#prettyPhoto/<?php echo $newsid; ?>/"><?php echo highlight($row->title, $keyword);?></a></strong></p>
					<p><?php echo highlight($row->description, $keyword);?></p>
				</div>
				<br/>
			<?php endforeach; ?>
		...
		<?php if(($blogs->num_rows() > 0)&&($keyword!="")):?>
			<?php foreach($blogs->result() as $row): ?>
				<div class="container thin">
					<p><strong><a href="<?=base_url();?><?php echo "news_blogs/post/".$row->id;?>"><?php echo highlight($row->title, $keyword);?></strong></a></p>
					<p><?php echo highlight($row->description, $keyword);?></p>
				</div>
				<br/>
			<?php endforeach; ?>
		...
		<?php if(($forum->num_rows() > 0)&&($keyword!="")):?>
			<?php foreach($forum->result() as $row): ?>
				<div class="container thin">
					<p><strong><a href="<?=base_url();?><?php echo "forum/post/".$row->section_id."/".$row->id;?>"><?php echo highlight($row->thread_name, $keyword);?></a></strong></p>
					<p><?php echo highlight($row->thread_description, $keyword);?></p>
				</div>
				<br/>
			<?php endforeach; ?>
		<?php else:	
