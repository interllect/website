...
class About extends CI_Controller {
	...
	$role = $data['role'];
		...
		elseif(($this->tank_auth->is_logged_in())&&($role >= '3')){
			...
			redirect('/admin/about/info', 'refresh');
		}
	}
	...
	$role = $data['role'];
		...
		elseif(($this->tank_auth->is_logged_in())&&($role >= '3')){
			...
			$data['about_count'] = $this->db->get('about');
			
			$number_results = $data['about_count']->num_rows();
			$this->load->library('pagination');
			$config['base_url'] = base_url().'admin/about/info/'.$data['order'].'/';
	...
	$role = $data['role'];
		...
		elseif(($this->tank_auth->is_logged_in())&&($role >= '2')){
			...
			$this->db->where('id', $data['id']);
			$data['about_edit'] = $this->db->get('about')->row();
			...
			else
			{
				...
				$this->db->where('id', $data['id']);
				$this->db->update('about', $item);
					
				redirect('admin/about/info_edit/'.$data['id']);
